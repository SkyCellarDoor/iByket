...
use App\WholesaleSellsProductModel;
...
class SellController extends Controller
{
    ...
    public function complete_wholesale(Request $request)
    {
        //dd($request);
        ...
        $new_sell = new WholesaleSellsModel();
        $new_sell->client_id = $client;
        $new_sell->use_bill = $use_bill;
        $new_sell->summa = $request->sum;
        $new_sell->discount = $request->promotion_sum;
...
//            }
            ...
            $update = new WholesaleSellsProductModel();
            $update->sells_id = $new_sell->id;
            $update->product_id = $product_id_sell;
            $update->amount = $value_one[$key];
            $update->amount_consist = $value_many[$key];
        ...
        return redirect()->route('opt_clients_list');
    }
    ...
    public function wholesale_sells_list()
    {

        $sells = WholesaleSellsModel::where('complete', 1)
            ->where('storage_id', Auth::user()->storage_id)
            ->get()->sortByDesc('id');

        return view('sell.wholesale_sells_list')->with(['sells' => $sells]);
    ...
    public function wholesale_sell_detail($id)
    {

        $sell = WholesaleSellsModel::find($id);
        $products = WholesaleSellsProductModel::where('sells_id', $id)->get();

        return view('sell.wholesale_sell_detail')->with(['sell' => $sell])->with(['products' => $products]);
