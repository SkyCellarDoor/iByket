...
class SetCostController extends Controller
{
    public function set_cost_complete(Request $request){
        ...
        foreach ($id_products as $id => $value) {
            ...
            $last_cost_one = $product->LastCostRetailOne();
            $last_cost_many = $product->LastCostRetailMany();
    ...
    public function set_cost_wholesale_complete(Request $request)
    {
        ...
        foreach ($id_products as $id => $value) {
            ...
            $last_cost_one = $product->LastCostWholeOne();
            $last_cost_many = $product->LastCostWholeMany();

            if (is_null($last_cost_one)) {

                $new_price = new CostWholesaleModel();
                $new_price->product_id = $product->id;
                $new_price->good_id = $product->good_id;
                $new_price->type = 1;
                $new_price->cost = $price_one[$id];
                ...
                $product->one_cost_sell_opt_id = $new_price->id;
                $product->save();
                ...
                $product->many_cost_sell_opt_id = $new_price->id;
                $product->save();

            } else {
                if ($last_cost_one->cost == $price_one[$id]) {

                    $product->one_cost_sell_opt_id = $last_cost_one->id;
                    $product->save();
                } else {
                ...
                if (!is_null($price_many[$id])) {
                    ...
                    if ($last_cost_many->cost == $price_many[$id]) {
                        ...
                        $product->many_cost_sell_opt_id = $last_cost_many->id;
                        $product->save();
                    } else {
    ...
    public function set_cost_wholesale(Request $request)
    {
        ...
        return view('price.set_price_wholesale')->with(['products' => $products])->with('back_url', $back_url);
