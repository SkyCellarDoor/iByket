...
@section('content')
    ...
    <form id="send_data" action="{{ route('complete') }}" method="post" class="ui form">
        {{ csrf_field() }}
        <div class="ui top attached menu" style="height: 48px;">
            ...
            @else
                <div class="item">
                    <a href="{{ route('detail_view') }}/{{ $client->id }}" target="_blank">{{ $client->name }}</a>
                    @if($client->bill >= 0)
