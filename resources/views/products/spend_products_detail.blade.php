...
@section('content')
    ...
    <div class="ui top attached menu" style="height: 48px;">
        <div class="item">&nbsp;Товар на списание</div>
        {{--<div class="item">Поставщик&nbsp;<b>{{ $invoice->user_provider_model->providers_model->company }}</b></div>--}}
    </div>

    <div class="ui bottom attached segment">
        <form action="{{ route('spend_product_complete') }}" method="post" class="">
            {{ csrf_field() }}
            <div class="ui grid">
                    ...
                    <div id="loader" class="ui segment">
                        <table id="result_table" class="ui compact celled table fluid" style="width: 100%">
                            ...
                            <tr>
                                ...
                                <th style="width: 1%" nowrap>Кол-во на списание</th>
                            ...
                            @foreach( $products as $product)
                                @if ($product->good_model->consist == 0)
                                    <tr>
                                        ...
                                        <td colspan="2" class="collapsing">
                                            {{ $product->cost_end }} p.
                                            <input data-id="{{ $product->id }}" name="value[]" class="cost"
                                                   type="hidden" value="{{ $product->cost_end }}">
                                        </td>
                                    </tr>
                                @else
                                    <tr>
